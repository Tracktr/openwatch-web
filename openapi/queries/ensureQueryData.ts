...
export const ensureUseApplicationsServiceGetApplicationsData = (queryClient: QueryClient) => queryClient.ensureQueryData({ queryKey: Common.UseApplicationsServiceGetApplicationsKeyFn(), queryFn: () => ApplicationsService.getApplications() });
export const ensureUseApplicationsServiceGetApplicationsByIdData = (queryClient: QueryClient, { id }: {
  id: string;
}) => queryClient.ensureQueryData({ queryKey: Common.UseApplicationsServiceGetApplicationsByIdKeyFn({ id }), queryFn: () => ApplicationsService.getApplicationsById({ id }) });
